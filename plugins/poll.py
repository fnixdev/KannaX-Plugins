...
async def vote_poll(msg: Message):
    ...
    if replied and replied.poll:
        ...
        try:
            await kannax.vote_poll(msg.chat.id, replied.message_id, option)
...
async def stop_poll(msg: Message):
    ...
    if replied and replied.poll:
        try:
            await kannax.stop_poll(msg.chat.id, replied.message_id)
        except Exception as e_f:
            await msg.err(e_f)
        else:
...
async def retract_vote(msg: Message):
    ...
    if replied and replied.poll:
        try:
            await kannax.retract_vote(msg.chat.id, replied.message_id)
        except Exception as e_f:
            await msg.err(e_f)
        else:
