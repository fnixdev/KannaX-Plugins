...
from kannax.plugins.bot.alive import _parse_arg
from kannax.plugins.bot.utube_inline import BASE_YT_URL, get_yt_video_id, get_ytthumb
from kannax.plugins.misc.upload import check_thumb
from kannax.plugins.utils.songlink import find_url_from_msg, get_song_link
...
class XPlayer(GroupCall):
    def __init__(self, chat_id: int):
        ...
        super().__init__(
            client=kannax, play_on_repeat=self.replay_songs, path_to_log_file=""
        )
...
async def get_groupcall(chat_id: int) -> XPlayer:
    if not vc_chats.get(chat_id):
        ...
        if kannax.has_bot:
            try:
                await kannax.get_chat_member(
                    chat_id, (await kannax.bot.get_me()).username
                )
...
async def play_now(gc: XPlayer) -> None:
    r = gc.playlist[0]
    key = r["id"]
    client = kannax.bot if r["has_bot"] else kannax
...
async def get_rawaudio_thumb(data: Dict) -> Optional[tuple]:
    key = data["id"]
    msg = data["msg"]
    client = kannax.bot if data["has_bot"] else kannax
    ...
    gc.playlist.append(
        {
            ...
            "yt_url": kwargs["yt_id"],
            "by_user": (await kannax.get_user_dict(m.from_user, attr_dict=True)).mention
            if m.from_user
            else None,
        }
    )
...
if kannax.has_bot:
    ...
    @kannax.bot.on_callback_query(filters.regex(pattern=r"^vcbtn_([a-z]+)$"))
    @check_owner
    ...
    @kannax.bot.on_callback_query(filters.regex(pattern=r"^vc_([a-z]+)_([-a-z+]+)$"))
    @check_owner
