...
from kannax.utils import progress, runcmd, take_screen_shot
...
async def fry_(message: Message):
    ...
    async with kannax.conversation(chat) as conv:
        ...
        media = await kannax.send_photo(chat, frying_file)
        await conv.get_response(mark_read=True)
        ...
        if response.photo:
            ...
            await kannax.download_media(message=response, file_name=deep_fry)
