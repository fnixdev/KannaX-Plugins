# made for kannax-X by @Kakashi_HTK(tg)/@ashwinstr(gh)
# v3.0.5
...
from kannax.utils import capitaled
...
async def dlist_(message: Message):
    ...
    result = await kannax.get_inline_bot_results(bot_, query_)
    if not result:
        await message.edit(
    ...
    try:
        async with kannax.conversation(message.chat.id, timeout=15) as conv:
