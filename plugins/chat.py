...
async def join_chat(message: Message):
    ...
    if not text:
        ...
        return
    try:
        chat = await kannax.get_chat(text)
        await kannax.join_chat(text)
        await kannax.send_message(text, f"```Joined {chat.title} Successfully...```")
...
@kannax.on_cmd(
    ...
    about={
        ...
        "examples": ["{tr}leave", "{tr}leave KannaXOt"],
    },
    allow_private=False,
)
async def leave_chat(message: Message):
    ...
    try:
        await kannax.send_message(text, "```Good bye, Cruel World... :-) ```")
        await kannax.leave_chat(text)
    except UsernameNotOccupied:
        await message.edit(
    ...
    if not user_id:
        try:
            ...
            if chat.type in ["group", "supergroup"]:
                link = await kannax.export_chat_invite_link(chat_id)
                await message.edit(
        ...
        try:
            await kannax.add_chat_members(chat_id, user_id)
...
async def stagall_(message: Message):
    ...
    message_id = replied.message_id if replied else None
    member = kannax.iter_chat_members(chat_id)
    async for members in member:
        if not members.user.is_bot:
            text += mention_html(members.user.id, "\u200b")
    await message.delete()
    await kannax.send_message(chat_id, text, reply_to_message_id=message_id)
...
async def set_chat(message: Message):
    ...
    if "-title" in message.flags:
        await kannax.set_chat_title(message.chat.id, args.strip())
        await message.edit("```Chat Title is Successfully Updated...```", del_in=3)
    elif "-uname" in message.flags:
        try:
            await kannax.update_chat_username(message.chat.id, args.strip())
        except ValueError:
        ...
        try:
            await kannax.set_chat_description(message.chat.id, args.strip())
        except BadRequest:
...
async def bots_in_chat(message: Message):
    admin, member = [], []
    async for bots in kannax.iter_chat_members(message.chat.id, filter="bots"):
        status = bots.status
        u_id = bots.user.id
