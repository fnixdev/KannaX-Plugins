...
from kannax import Message, filters, get_collection, pool, kannax
from kannax.utils import get_file_id

LOGGER = kannax.getCLogger(__name__)
...
async def _init():
    ...
    if CUSTOM_REPLY_CHANNEL:
        async for message in kannax.iter_history(
    ...
    # Group Features Won't be displayed in Help Info For Now 😉
        ...
        for chat_id in ACTIVE_CHATS:
            if not str(chat_id).startswith("-100"):
                continue
            chat_ = await kannax.get_chat(chat_id)
            title = chat_.title
            msg += f"{title} {chat_id}\n"
        ...
        for user_id in ACTIVE_CHATS:
            if str(user_id).startswith("-100"):
                continue
            try:
                u_info = await kannax.get_user_dict(user_id)
...
async def lydia_ai_chat(message: Message):
    ...
    if data:
        ...
        try:
            out = ""
            await kannax.send_read_acknowledge(
...
# Idea arised from here (https://t.me/kannaxot/157629) thnx 👍
