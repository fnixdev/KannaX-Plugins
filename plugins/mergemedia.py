...
from kannax.plugins.misc.upload import upload
from kannax.utils import progress, runcmd, safe_filename
...
async def merge_(message: Message):
    ...
    for media in os.listdir("kannax/xcache/merge"):
        data_ = "file" + " " + "'" + "kannax/xcache/merge/" + media + "'" + "\n"
        txt_file.write(data_)
    txt_file.close()
    # detecting extension.
    await message.edit("`😎🥲 detecting extension ...`")
    for ext in os.listdir("kannax/xcache/merge")[:1]:
        rege_x = re.findall("[^.]*$", ext)[0]
        await message.edit(f"detected extension is .{rege_x}")
    # custom name.
    if name_:
        output_path = "kannax/xcache/merge/" + name_ + "." + rege_x
    else:
        output_path = "kannax/xcache/merge/output." + rege_x
    # ffmpeg.
    ...
    shutil.rmtree("kannax/xcache/merge")
    os.remove("merge.txt")
...
async def mergeclear_(message: Message):
    """Clear Saved."""
    try:
        shutil.rmtree("kannax/xcache/merge", ignore_errors=True)
